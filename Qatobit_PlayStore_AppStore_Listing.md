# Qatobit — Mobile App Store Listing Pack (Play Store + App Store)

**Prepared for:** Mobile app submission (Google Play Store & Apple App Store)
...
| 7 | **Copyright Information** | `© 2026 Qatobit Private Limited. All rights reserved.` |
| 8 | **Content Rating** | Target **Rated for 3+ / Everyone** (Play) and **17+** (Apple — Apple auto-assigns 17+ to apps facilitating crypto trading). Honest questionnaire answers in Section 6. No gambling, no UGC, simulated/real financial transactions = yes. |
| 9 | **Data Safety / Privacy** | Full declaration matrix in **Section 7** (collected vs. shared, encryption in transit, deletion request path). Privacy policy URL required: `https://qatobit.com/privacy`. |
| 10 | **Release Notes** | v1.0.0 launch copy in **Section 3** (plus a reusable template for future updates). |
| 11 | **Contact Information** | Qatobit Private Limited, Chennai, Tamil Nadu, India · `support@qatobit.com` **[confirm]** · `https://qatobit.com`. Developer-account legal name + address must match your incorporation record. |

---

## 2. Full Description (2,447 / 4,000 characters)

> Voice: precise, confident, grounded, warm. Methodology-first. Keywords placed naturally in the first two lines and sub-headers (where store algorithms weight them most).

```
Qatobit is India's Crypto Wealth Architect — a crypto wealth management platform for investors who treat crypto as a portfolio allocation decision, not a gamble. Wealth, by design.

WHY METHODOLOGY COMES FIRST
You're the kind of investor who reads the methodology before the returns. So are we. Every QSI Crypto Index is built on documented construction logic, institutional-grade data, and published inclusion and exclusion criteria — and rebalanced monthly, with discipline. You can see exactly how each index is built before you invest a single rupee.

THE QSI CRYPTO INDICES
• QSI Core — broad, large-cap exposure for a stable foundation
• QSI Growth — higher-conviction allocation built for long-term compounding
• QSI VRION — a volatility-aware strategy index
• QSI GEQ10 — tokenized equity exposure (GEQ10)
Each index has its own methodology page. Read it. Then decide.

INVEST THE WAY DISCIPLINED INVESTORS DO
• Crypto SIP — automate steady, recurring investments and let time compound for you
• Quick Buy / Sell — move in and out in seconds, with transparent pricing
• Real-World Assets (RWAs) & Tokenized Stocks — diversify beyond crypto, on-chain
...
The conversation around crypto is loud. Qatobit is the structured alternative — methodology you can read, a portfolio you can explain, and the tools to act with intent.

Understand the methodology. Trust the platform. Invest with design.

Download Qatobit and build crypto into your wealth — by design.
...
**v1.0.0 — Launch (ready to paste):**
```
Welcome to Qatobit — India's Crypto Wealth Architect.
• Invest in QSI Crypto Indices: Core, Growth, VRION & GEQ10 — each with a published methodology
• Start a Crypto SIP and invest on a schedule
...
**Reusable template for future updates** (keep ≤500 chars, lead with the user benefit):
```
What's new in this version:
• [Headline feature — what the investor can now do]
• [Improvement — faster/clearer/safer]
• Bug fixes and performance improvements.
Questions? support@qatobit.com
```

---

## 4. Recommended Titles & Subtitles (the highest-impact ASO fields)
...
## 5. Keyword Research & Strategy (data-backed)

All search-volume and competition figures below are from **Semrush, India (`in`) database, June 2026**. Competition is the paid-difficulty index (0–1); for organic/ASO difficulty, low absolute results + high volume = best opportunity. These are **web** search volumes — the best available proxy for App Store/Play search demand, which the stores don't publish.

### 5a. Core keyword set — volume vs. competition

| Keyword | Monthly Volume (IN) | Competition | Read |
|---|---|---|---|
| crypto app | **27,100** | 0.08 | Huge volume, low competition — must-win head term. |
| crypto trading app | 6,600 | 0.18 | High intent, moderate competition. |
| crypto exchange india | 5,400 | 0.01 | High intent + very low competition — strong target. |
| best crypto app | 1,300 | 0.07 | Comparison intent; win via ratings + "best" in reviews. |
| bitcoin app | 1,300 | 0.28 | Broad entry term. |
| cryptocurrency investment | 1,000 | 0.04 | Matches Qatobit's positioning exactly. |
| invest in crypto | 880 | 0.03 | Intent-rich, low competition. |
| crypto investment | 880 | 0.03 | Core to brand; low competition. |
| crypto index | 880 | 0.33 | **Category-defining for Qatobit** — own it early. |
| crypto investment app | 590 | 0.09 | Exact product-category match. |
| crypto mutual fund | 480 | 0.05 | Adjacent mental model for SIP/index investors. |
| crypto for beginners | 110 | 0.02 | Feeds Cluster 1 (first-time allocator). |
| crypto index fund | 90 | 0.01 | Near-zero competition; high relevance. |

### 5b. Competitor brand demand (the real ASO battleground)

| Brand | Monthly Brand Volume (IN) | Note |
|---|---|---|
| coindcx | **246,000** | Market leader by search demand. |
...
### 5c. Emerging terms (low/no volume today — plant the flag)

`crypto sip`, `crypto wealth`, `tokenized stocks`, `real world assets`, `passive crypto investing` register little/no India web volume yet. That's the opportunity: these map directly to Qatobit's products, so seeding them in the description now means Qatobit ranks first as demand grows. Reinforce with content/SEO on qatobit.com to create the demand.

### 5d. Field-by-field keyword placement

- **Apple keyword field (100 chars):** `crypto,bitcoin,index,sip,invest,trading,exchange,portfolio,ethereum,usdt,wealth,fund,btc,coin` — no spaces (saves characters), singular forms, no words already in the title/subtitle (Apple indexes title+subtitle+keywords as one set, so never duplicate).
- **Play Store (no keyword field):** the algorithm reads the **title, short description, and full description**. Each high-value keyword (crypto app, crypto exchange india, crypto index, crypto investment app, crypto SIP) appears naturally in the copy above. Aim for ~2–3 repetitions of the head term across the description — present, never stuffed.
...
| Gambling (real or simulated) | **No** — investing is not gambling; do not mislabel. |
| Users can purchase real items / trade real currency or assets | **Yes** (crypto + tokenized assets) |
| User-generated content / social features | No (unless community features ship) |
| Shares user location | Only if used; declare accurately |
| **Expected result** | Play: **Everyone / 3+**. Apple: typically **17+** (auto-applied to crypto-trading apps). |

---

## 7. Data Safety / Privacy Declaration

Privacy policy URL (**required, must be live before submission**): `https://qatobit.com/privacy`

For a KYC-based crypto wealth app, expect to declare the following. Confirm against your actual SDKs/backend before submitting — **inaccurate declarations are the #1 cause of crypto-app rejections.**
...
| App usage / interactions | Yes | Analytics provider | Product analytics | e.g. PostHog/Mixpanel |
| Approx. location | If used | — | Fraud/region compliance | Declare only if used |
| **Security practices** | — | — | — | **Data encrypted in transit (TLS); users can request account & data deletion via support@qatobit.com [confirm flow]** |

State affirmatively: data is encrypted in transit; users can request deletion; the app follows the Play Families/financial-data policies. (Qatobit's CERT-In audit and Proof of Reserves support these claims.)
...
## 9. Competitive Advantage & Go-to-Market Strategy (summary)
...
- [ ] **Data Safety matrix** verified against actual SDKs (analytics, KYC vendor, payment partners).
- [ ] **Confirm the 5 screenshots** in Section 8 with the design team.
